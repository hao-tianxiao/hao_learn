   ...
   if (this->count == 0)
   {
    double raw, pitch, theta;
     ...
     tf::Matrix3x3(q).getRPY(raw, pitch, theta);
      std::string roadMap_path = "/home/hao/hao_learn/src/path_record/data/road_path_by_gps.txt";//地图录制位置
      FILE *fp_s;
      fp_s = fopen("/home/hao/hao_learn/src/path_record/data/road_path_by_gps.txt","a");
  ...
  p='c';
   if (this->count == 0)
   {
    double raw, pitch, theta;
     ...
     tf::Matrix3x3(q).getRPY(raw, pitch, theta);
      std::string roadMap_path = "/home/lwh/Lpp_files/data/road_path_by_gps.txt";//地图录制位置
      FILE *fp_s;
      fp_s = fopen("/home/lwh/Lpp_files/data/road_path_by_gps.txt","a");
...
void path_record::record_path2(const nav_msgs::Odometry::ConstPtr odometry_msg){
    ...
    tf::Matrix3x3(q).getRPY(raw, pitch, theta);
    FILE *fp_s;
    fp_s = fopen("/home/hao/hao_learn/src/path_record/data/road_path_odom1.txt","a");
    // fprintf(fp_s, "%c %lf %lf", p, odometry_msg->pose.pose.position.x, odometry_msg->pose.pose.position.y);  // Angle
    fprintf(fp_s, "%lf %lf", odometry_msg->pose.pose.position.x, odometry_msg->pose.pose.position.y);  // Angle
    fprintf(fp_s, "\r\n");  
    fclose(fp_s);
    loop_rate.sleep();
...
void path_record::nodeStart(int argc, char **argv){
    ...
    count = 0;
    // 订阅相关节点
    ros::Subscriber sub_odom = nc.subscribe("/odom", 1, &path_record::record_path2, this);//录制轨迹话题雷达
    // ros::Subscriber sub_odom = nc.subscribe("/odom_by", 1, &path_record::record_path, this);//录制轨迹话题小车里程计
    //  ros::Subscriber sub_odom = nc.subscribe("/rear_post", 1, &path_record::record_path_GPS, this);//录制轨迹话题GPS
    //  ros::Subscriber sub_odom = nc.subscribe("/Odometry", 1, &globle_planning::odometryGetCallBack_slam, this); // 控制节点gps
      ros::spin();
}
